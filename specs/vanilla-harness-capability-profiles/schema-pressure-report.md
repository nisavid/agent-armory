...
| SP-003 | accepted | all | Skills, plugins, subagents, commands, MCP, hooks, and config surfaces all need load path, activation, scope, and reload detail, for example [Claude Code skills](https://code.claude.com/docs/en/skills) and [Cursor plugins](https://cursor.com/docs/plugins). | `[[claim.detail]] component = "skills"; load_scope = ["project", "user"]; activation = ["implicit", "explicit"]` | For refreshed claims, require component, load or attachment point, activation, mutability, scope, and evidence references when known. | Profile enrichment adds nested detail tables while preserving stable family claim IDs. |
