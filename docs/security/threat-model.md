...
- Validation tooling that future agents use to decide whether Forge surfaces
  are complete.
- Issue Tracker Ops metadata in GitHub Issues, including labels, issue state,
  comments, triage records, dependency relations, and audit output that agents
  use for delegation and issue selection.
...
- Repo content to external control surfaces: issue projection, PR bodies, and
  ...
  repository.
- Local Issue Tracker Ops tooling to GitHub: `tools/issue_tracker_ops.py` uses
  the authenticated `gh` CLI to read issue metadata, create or update issues,
  post comments, and manage dependency relations. Mutation modes require
  explicit `--execute`; read modes still cross a network and authentication
  boundary.
...
- The GitHub Issues baseline treats labels as structured custom predicates and
  comments as rationale records until richer tracker or GitHub Projects support
...
- Triage comments and issue records that are generated by AI must be marked as
  such and must not publish private session context, raw logs, secrets, or
  instance-scoped evidence unless that disclosure has been classified and
  approved.
- Issue labels used as custom predicates must preserve their documented
  cardinality and meaning; label drift must not silently change readiness,
  delegation, dependency, or issue-selection decisions.
...
- Closeout evidence, security scan summaries, issue projection text, and review
  dispositions.
- Issue labels and triage records that represent readiness, triage depth, work
  kind, engagement mode, brief status, dependency disposition, or rationale for
  agent delegation.
...
- `tools/issue_tracker_ops.py` defaults mutation-capable operations to dry-run
  output, uses `subprocess.run` with an argument list, sends JSON payloads on
  stdin, and summarizes GitHub results instead of emitting verbose HTTP logs.
- `audit-labels` provides a read-only check for missing or conflicting baseline
  label axes before agents rely on issue metadata for bulk triage, delegation,
  or issue selection.
- `docs/agents/triage-labels.md` separates labels from triage-record comments
  and instructs agents to keep private or instance-scoped evidence out of public
  tracker content.
...
- A malformed or conflicting set of issue labels makes an issue appear
  ready-for-agent or unblocked when the underlying rationale still requires
  human review.
- An AI-generated triage comment leaks private session details, raw local paths,
  or speculative internal reasoning into a public issue.
