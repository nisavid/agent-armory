...
- Config runtime -> MCP surface: `mcp_tool_definitions()` and
  `call_mcp_tool()` expose typed parity for Config read, authoring,
  onboarding, migration-preview, migration-apply, and reviewed plan-artifact
  apply operations. Controls include closed-world input schemas, read/write
  classification, per-call authority for local writes, reviewed-plan-only
  `config.apply` input, and CLI-equivalent redaction.
